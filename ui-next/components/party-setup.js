...
export class PartySetupController {
  ...
  mount() {
    ...
    this.#statsSettingsPanel = new StatsSettingsPanel({
      ...
      resolveBuildStats: (buildId, slotIndex, mode, characterLevel, styleLevel) => {
        ...
        return this.#resolveAutomaticStats(slotIndex, mode, equipConfig, characterLevel, styleLevel);
      },
      onChange: (slotIndex, mode, stats, characterLevel, styleLevel) => {
        const slot = this.#slots[slotIndex];
        if (!slot) return;
        if (mode === 'support') {
          ...
          if (styleLevel !== undefined) slot.styleLevel = styleLevel;
        }
        this.#render();
        this.#notifyChange();
      },
  ...
  getSnapshot() {
    ...
    return {
      ...
      characterLevelsByPartyIndex: Object.fromEntries(
        this.#slots.map((s, i) => [i, s.characterLevel ?? 180])
      ),
      styleLevelsByPartyIndex: Object.fromEntries(
        this.#slots.map((s, i) => [i, s.styleLevel ?? 20])
      ),
  ...
  applySnapshot(snapshot = {}) {
    ...
    for (let i = 0; i < PARTY_SLOT_COUNT; i++) {
      ...
      if (slot.style) {
        slot.defaultStats = this.#resolveAutomaticStats(i, 'main');
        slot.stats = normalizeCharacterStats(snapshot?.statsByPartyIndex?.[i]?.stats) ?? null;
      }
      if (slot.style && slot.supportStyle) {
        slot.supportDefaultStats = this.#resolveAutomaticStats(i, 'support');
        slot.supportStats = normalizeCharacterStats(snapshot?.statsByPartyIndex?.[i]?.supportStats) ?? null;
      }
    }
  ...
  #loadPreset(index) {
    ...
    for (let i = 0; i < PARTY_SLOT_COUNT; i++) {
      ...
      if (slot.style) {
        slot.defaultStats = this.#resolveAutomaticStats(i, 'main');
        slot.stats = normalizeCharacterStats(s?.stats) ?? null;
      }
      if (slot.style && slot.supportStyle) {
        slot.supportDefaultStats = this.#resolveAutomaticStats(i, 'support');
        slot.supportStats = normalizeCharacterStats(s?.supportStats) ?? null;
      }
    }
    this.#syncReorderMode();
    this.#render();
