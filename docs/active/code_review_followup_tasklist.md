...
- 2026-03-08: `R-006` の replay session 側では `runInTurnPlanReplaySession()` を追加し、`recalculateTurnPlans()` / `stageTurnPlanSession()` の `isReplayingTurnPlans` 切替と UI 再描画を共通化した。`serializeRecordField()` も `stringifyRecordFieldValue()` を経由するようにし、`JSON.stringify` 失敗時は `String(value)` へ安全にフォールバックすることを DOM テストで固定した。
- 2026-03-08: `R-006` の browser boundary 側では `getBrowserConfirmHandler()` / `parseJsonText()` / `buildTextDownloadTarget()` / `cleanupTextDownloadTarget()` を追加し、confirm fallback、scenario JSON parse、records JSON download cleanup の低レベル処理を helper へ寄せた。anchor click 失敗時でも object URL が revoke されることを DOM テストで固定した。
- 2026-03-08: `R-006` の selection save 側では `normalizeSelectionStoreSnapshot()` を追加し、legacy manual-slot 配列から現行 slot layout への移行と、schema 不整合時の empty store fallback を `readSelectionStore()` から切り出した。legacy slot 変換と invalid schema fallback は UI テストで固定した。
- 2026-03-08: `R-006` の scenario cursor 側では `getScenarioTurns()` / `hasRemainingScenarioTurns()` / `applyScenarioTurnAtCursor()` / `finalizeScenarioTurnProgress()` / `finalizeScenarioCompletion()` を追加し、`stageCurrentScenarioTurn()` / `runNextScenarioTurn()` / `runAllScenarioTurns()` の重複していた cursor 更新と status 表示を helper へ寄せた。stage / executed / completed 文言は DOM テストで固定した。
- 2026-03-08: テスト基盤のブロッカーとして `json/reports/migration/new_skill_database.draft.json` 不在時に DOM suite が全落ちしたため、`HbrDataStore.fromJsonDirectory()` は migration draft を optional read に変更した。schema validation テストもディスク上の report を読まず、テスト内の sample document で `valid` / `invalid` / missing-required を検証する形へ切り替えた。
- 2026-03-08: `R-006` の scenario apply 側では `applyScenarioTurnStateOverrides()` / `applyScenarioEnemyStatusDelta()` / `applyScenarioPreemptiveOverdrive()` / `applyScenarioQueuedSwaps()` / `applyScenarioQueuedActions()` を追加し、`applyScenarioTurn()` の enemy status 更新・OD開始・swap/action override の深い分岐を helper へ寄せた。scenario 経由の enemy status apply/clear も DOM テストで固定した。
- 2026-03-08: `R-006` の scenario 実行末尾では `applyScenarioInterruptOverdrive()` / `clearScenarioStagePreviewState()` / `buildScenarioTurnCommitOptions()` / `executeScenarioTurnResolution()` を追加し、`applyScenarioTurn()` の interrupt OD 反映、stage 時の preview 後始末、`commit:false` preview 分岐を helper へ寄せた。scenario stage の interrupt 予約維持と preview-only 実行も DOM テストで固定した。
- 2026-03-08: `R-006` の turn-plan mutation 側では `resolveTurnPlanIndex()` / `finalizeTurnPlanMutation()` を追加し、`saveTurnPlanEditFromDom()` / `cancelTurnPlanEdit()` / `deleteTurnPlanRow()` / `moveTurnPlanRow()` の index 検証と再計算後始末を共通化した。cancel edit 時の edit session clear と再計算再実行も DOM テストで固定した。
- 2026-03-08: `R-006` の edit session 前段 replay では `resetTurnPlanReplayDiagnostics()` を追加し、`replayTurnPlansBeforeIndex()` も `replayTurnPlanAtIndex()` の force fallback/warning 経路を再利用する形に揃えた。先行 turn が壊れていても force mode なら後続 row の編集開始まで進めることを DOM テストで固定した。
