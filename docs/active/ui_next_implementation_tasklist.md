...
>
> **進捗サマリー**: T01 ✅ / T02 🔶 / T03〜T12 ✅（T12-E-3まで） / T13〜T19 ❌ 未着手（2026-03-16 ギャップ分析から追加）
>
> **前提設計**:
...
- [ ] `formatSkillCostLabel()` を `ui-next/` に移植（SP/Token/Morale/EP/ALL・鬼神化中SP0 対応）
...
- [ ] `turn-row.js` の skill select option ラベルに組み込む
- [ ] SP コスト変化（装備 / バフによる補正後値）を反映する

完了条件:
- [ ] スキル select に `SP N` / `Token N` / `EP N` / `SP ALL` 等のコスト種別が表示される
- [ ] 鬼神化中スロットで SP コストが `SP 0` になる

#### T13-B: スキル非表示フィルタ UI 【優先度: 高 / 操作性直結】

**目的**: 使わないスキルをリストから非表示にして選択ミスを減らす

**UX フロー**:
1. Party Setup の各スロットに「🔧 スキル絞込」ボタンを追加
2. クリックでフローティングパネルを表示（既存 UI の上に重ねる、z-index 上位）
3. パネル内容: そのスタイルが持つスキル全件 × チェックボックス付きラベル
4. チェックを外したスキルは action select から非表示になる（`disabled` でなく `display:none`）
5. 「全選択 / 全解除」ショートカットボタンを付ける
6. 設定は `styleId` キーで `localStorage` に保存（ページリロード後も維持）
7. スタイル変更時にフィルタ設定をリセット（新スタイルの全スキルを初期表示）

**実装箇所**:
- `party-setup.js`: 各スロットに「🔧」ボタンを追加、クリックで `SkillFilterPanel` を開く
- `ui-next/components/skill-filter-panel.js` を新規作成（フローティングパネル）
- `turn-row.js`: `buildSkillOptions()` でフィルタ設定を参照して option を除外
- フィルタ設定は `ui-next/engine/turn-engine-manager.js` 経由ではなく、slot ごとの `localStorage` から直接参照（純粋な表示設定のため）

- [ ] `party-setup.js`: 各スロット下に「🔧 スキル絞込」ボタン追加
- [ ] `ui-next/components/skill-filter-panel.js` 新規作成
  - フローティングパネル（fixed 配置 / backdrop なし）
  - スキル一覧 checkboxLabel × N（スキル名・コスト種別を表示）
  - 「全選択 / 全解除」ボタン
  - パネル外クリックで閉じる
- [ ] フィルタ設定を `localStorage` の `hbr.skill_filter.{styleId}` キーで保存
- [ ] `turn-row.js` の skill select 構築時にフィルタ設定を参照して非対象スキルを除外

完了条件:
- [ ] スロットの「🔧」ボタンを押すとフローティングパネルが表示される
- [ ] チェックを外したスキルが action select から消える
- [ ] ページリロード後もフィルタ設定が維持される

#### T13-C: スキル属性バッジ + スイッチスキル対応 【優先度: 中 / 同時実装推奨】

**スキル属性バッジ**:
- [ ] select 横に属性バッジコンテナを追加（斬・刺・打 + 火・氷・雷・闇・光）
- [ ] スキル変更時に `resolveEffectiveSkillForAction()` で有効スキルの `attackType` / `elements` を取得

**スイッチスキル対応**:
> UIは要検討。複数バリアントがある場合は「2スキルを縦に並べる」簡易実装も許容。
- [ ] `resolveEffectiveSkillForAction()` で現在の有効バリアントを取得
- [ ] スイッチスキルの場合: select の代わりに（または直下に）もう一方のバリアントをラベル表示
  - シンプル案: 2行縦並び（有効バリアント選択 + 非有効バリアント参照表示）
  - 属性バッジはバリアントごとに別バッジで並置
- [ ] スイッチ状態変化後に両バリアントのバッジを更新

完了条件:
- [ ] スキル select の横に属性バッジが表示される
- [ ] スイッチスキルで 2 バリアントの属性が識別できる
...
- [ ] キャラアイコン下に status effects バッジ一覧を追加
  - `member.resolveEffectiveStatusEffects()` を参照して `statusType(duration)` 形式で表示
  - SP 増減・OD ゲージ増減に関わるエフェクトを視覚的に強調（色分け等）
  - 0件の場合は非表示
- [ ] 鬼神化中・行動不能バッジ
  - キャラアイコン下に `鬼神化中: 残りNターン` / `行動不能: 残りNターン`
  - `member.isReinforcedMode` / `member.actionDisabledTurns` を参照

#### T14-B: DP 表示 【優先度: 中】
- [ ] SP バッジに加え DP バッジを追加（`currentDp / baseMaxDp` 表示）
- [ ] `member.dpState` から `formatDpStateSummary()` 相当を実装

完了条件:
- [ ] 各キャラアイコン下でバフ/DP/鬼神化状態が確認できる
...
- [ ] Enemy Setup タブに敵数 select（1〜N）を追加
- [ ] 各敵ごとに入力セクションを動的生成
  - 敵名 input
  - 5属性ダメージレート（火氷雷闇光）× 各 input[type=number]
  - 破壊率 input[type=number]
- [ ] 変更時に `BattleStateManager` の初期設定に反映する
...
> **依拠**: ギャップ分析 分類A — CSV Export 系
> **参照**: `docs/active/ui_next_old_feature_gap_analysis.md` §A-6
> **旧実装**: `dom-adapter.js:L4010/L8548`

#### T17-A: CSV エクスポートボタン
- [ ] ターンリスト上部にエクスポートボタンを追加
- [ ] `TurnEngineManager.exportCsvState()` を呼び出し CSV 文字列取得
- [ ] Blob/URL API でブラウザダウンロード（`buildTextDownloadTarget()` パターン流用）

#### T17-B: CSV 列構成（Simple モード）
- [ ] Simple モード: `turnId / turnLabel / 前衛3スロット×(キャラ名+スキル名)` の最小列
- [ ] `getRecordColumns()` (L8548) の Simple 列定義を参照して実装
- [ ] Full モード切り替えは後続タスクとする

#### T17-C: JSON エクスポート
- [ ] 「JSON 保存」ボタンを追加
- [ ] `ReplayScript` を JSON シリアライズしてダウンロード

完了条件:
- [ ] ボタンクリックで CSV ファイルがダウンロードされる
- [ ] Simple モードの列構成が正しく出力される

> ❌ T17 未着手

---

### T18: Scenario Runner UI 実装

> **依拠**: ギャップ分析 分類A — Scenario Runner 系
> **参照**: `docs/active/ui_next_old_feature_gap_analysis.md` §A-7
> **旧実装**: `dom-adapter.js:L4603/L4768/L5937`

#### T18-A: シナリオ入力 UI
- [ ] シナリオパネル（テキストエリア + 実行ボタン）を追加
- [ ] JSON / CSV を自動判別して解析

#### T18-B: TurnEngineManager へのシナリオ実行連携
- [ ] `convertCsvToScenario()` を移植（ODコンテキスト復元・swaps逆算含む）
- [ ] `TurnEngineManager.loadAndRunScenario(scenarioJson)` を追加
  - 旧 `applyScenarioTurn()` の 8 ステップフローを参考
  - エンジン関数経由での state 適用（直接 state 変更禁止）

#### T18-C: シナリオ進捗表示
- [ ] 実行済みターン数 / 総数をステータスバーに表示

完了条件:
- [ ] JSON/CSV シナリオを貼り付けて実行するとターンが順次再生される

> ❌ T18 未着手

### T19: スキル使用回数（use_count）表示 【優先度: 低 / 複雑なため独立】
