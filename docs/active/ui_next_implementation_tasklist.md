...
- [ ] 選択確定時に同一キャラクター重複を排除する（下記ルール）
  - メイン同士: 同一キャラ不可 → 既存スロットをクリア
  - サポートへのメインキャラ選択: 不可 → 既存スロットをクリア
  - サポート同士: 同一キャラ不可 → 既存スロットをクリア
  - （パーティ全体で各キャラクターはいずれか1スロットのみ）

完了条件:

- slot ごとに main / support style を意図した操作モデルで差し替えられる
- パーティ全体（メイン×6 ＋ サポート×6）で同一キャラクターが重複しない
