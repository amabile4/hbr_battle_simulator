...
> 🔶 T20 部分完了（2026-03-17）: A/B/C 実装済み。D のタッチ交代（swap）は実装済み（2026-04-01確認）。
> ⚠️ デスクトップ向け Layout Rework（T22以降）後にモバイルレイアウトが大幅に崩れているため、T20-E として全般見直しを追加。
...
## 優先度サマリ（2026-04-01 更新）

本シミュレータは SP/OD 計算機であるため、下記の優先順で実装する。

| 優先度 | タスク | 理由 |
|--------|--------|------|
| ✅ | T20-A/B iOS viewport/セーフエリア | 使用不能バグ（ボタンが押せない） |
| ✅ | T13-A スキルコスト詳細 | SP 消費量がリアルタイムで見える |
| ✅ | T13-B スキル非表示フィルタ | 操作ミス防止・選択精度向上 |
| ✅ | T20-C モバイルレイアウト | 375px 以下で画面が使えない |
| **高** | T20-E モバイル UI 全般見直し | デスクトップ調整後に大幅に崩れている |
| ✅ | T15 鬼神化 | OD +15% の計算精度に直結 |
| ✅ | T14-A バフ/デバフ・鬼神化バッジ | SP/OD 影響ステータスの可視化 |
| **高** | T16 Enemy Setup | 正しい敵設定なしに OD 計算が完結しない |
| ✅ | T13-C 属性バッジ（バッジ部分） | 戦術判断の補助 |
| ✅ | T17 JSON Export | ~~CSV Export 破棄~~ JSON Export で完了 |
| ✅ | T18 Scenario Runner | ~~CSV 再生・検証用~~ JSON 読み込みで代替完了 |
| **低** | T20-D タッチ UX（swap 以外） | swap 実装済み、残はタップターゲット等 |
