...
> ✅ T13-B 完了（2026-03-17）: `ui-next/utils/skill-filter.js`・`ui-next/components/skill-filter-panel.js` 新規作成、`party-setup.js`・`turn-row.js`・`turn-area.js` 修正。既存 630 テスト PASS。
...
- [x] `party-setup.js`: 各スロット下に「🔧 スキル絞込」ボタン追加
- [x] `ui-next/components/skill-filter-panel.js` 新規作成
  - フローティングパネル（fixed 配置 / backdrop なし）
  - スキル一覧 checkboxLabel × N（スキル名・コスト種別を表示）
  - 「全選択 / 全解除」ボタン
  - パネル外クリックで閉じる
- [x] フィルタ設定を `localStorage` の `hbr.ui_next.skill_filter.v1` キーで保存
- [x] `turn-row.js` の skill select 構築時にフィルタ設定を参照して非対象スキルを除外

完了条件:
- [x] スロットの「🔧」ボタンを押すとフローティングパネルが表示される
- [x] チェックを外したスキルが action select から消える
- [x] ページリロード後もフィルタ設定が維持される
...
> ✅ T13-C スキル属性バッジ 完了（2026-03-17）: `turn-row.js` に `ATTACK_TYPE_MAP`/`ELEMENT_MAP` 定数・`#buildSkillBadgesHtml`・`#updateSkillBadges` 追加。`#buildFrontSlotHtml`・`#bindEvents`・`refreshSkillSelects` にバッジ更新を組み込み。既存 630 テスト PASS。スイッチスキル対応は別タスク（要検討）。
...
> 🔶 T20 部分完了（2026-03-17）: A/B/C 実装済み。D（タッチ UX）は後回し。
