...
### T32: Stage Setup 完全実装

> **現状**: T10 で差し込み構造のみ実装済み。`Initial Setup > Stage Setup` はまだ TODO placeholder のまま。

#### T32-A: Stage Setup スコープ整理
- [ ] 旧UI / shared engine / 現行設計から Stage Setup で扱う初期 state を棚卸しする
- [ ] UI Next で扱う項目を確定する（zone / territory / talisman / その他 stage 側 state）

#### T32-B: Stage Setup UI
- [ ] `Stage Setup` タブにフォームを実装する
- [ ] setup snapshot の保存・復元経路へ追加する

#### T32-C: BattleState 接続
- [ ] `BattleStateManager` の初期 state へ stage setup を反映する
- [ ] unit / integration test を追加する

完了条件:
- [ ] `Stage Setup` タブで stage 側初期設定を入力できる
- [ ] Apply 後に stage 設定が BattleState に反映される

> ❌ T32 未着手（T10 は extension point のみ完了）
...
| **高** | T32 Stage Setup 完全実装 | Stage Setup タブが placeholder のままで、初期条件を十分に設定できない |
