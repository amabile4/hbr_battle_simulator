...
>
> **進捗サマリー**: T01 ✅ / T02 🔶（最小化 未） / T03 ✅（T08で完了確認） / T04 ✅ / T05 ✅ / T06 ✅ / T07 ✅ / T08 ✅ / T08-UX ✅ / T09-Engine ✅ / T09-UI ✅ / T10 ✅ / T11 ✅ / T12 🔶（基盤・UX改善済み）/ T13〜T18 ❌ 未着手（2026-03-16 ギャップ分析から追加）
>
> **前提設計**:
...
- [ ] 鬼神化中（`member.isReinforcedMode`）の SP0 表示を確認

#### T13-B: スキル属性バッジ
- [ ] select 横に属性バッジコンテナを追加（斬・刺・打・属性アイコン）
- [ ] `resolveEffectiveSkillForAction()` で有効スキルの属性情報を取得して表示

#### T13-C: スキル使用回数残り (use_count)
- [ ] `member.getSkillUsageRemaining(skillId)` 相当を参照して select option に `残りN回` を付記
- [ ] 残り0回のスキルを `disabled` 表示にする

#### T13-D: スイッチスキル有効バリアント表示
- [ ] select の表示スキル名を `resolveEffectiveSkillForAction()` の有効バリアントに同期
- [ ] スイッチ状態変化後に select を再描画する

完了条件:
- [ ] スキル select に `SP N` / `Token N` 等のコスト種別が表示される
- [ ] use_count のあるスキルに `残りN回` が表示される
...
#### T14-C: 鬼神化中・行動不能バッジ
- [ ] キャラアイコン下に鬼神化バッジ（`鬼神化中: 残りNターン` / `行動不能: 残りNターン`）
