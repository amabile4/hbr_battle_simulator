...
>
> **進捗サマリー**: T01 ✅ / T02 ✅ / T03〜T12 ✅（T12-E-5 まで） / T13-A ✅ / T13-B ✅ / T13-C ✅ / T14 🔶（A完了・B=計算機シミュWIP） / T15 ✅ / **T16 🔶 Enemy Setup（敵スロット選択・実効倍率％耐性・吸収・max_d_rate・od_rate 初期反映）** / T17〜T19 ❌ 未着手 / **T20 🔶 iOS レスポンシブ対応（A/B/C 完了・D 未着手）** / **T21 ✅ Passive Debug Log** / **T22 ✅ Layout Rework** / **T23 ✅ PNG Capture Rework** / **T24〜T28 ✅ toolbar / D&D / legacy cut / log pane resize / manual-break E2E** / **T29 ✅ Enemy先制フィールド表示（文章のみ）** / **T30 ✅ Session JSON 旧フォーマット互換（styleIds=0）** / **T31 ✅ SP>=0条件＋速弾きの合算使用可否実装**
>
> **前提設計**:
...
#### T14-B: DP 表示 【スコープ外 / 計算機シミュ WIP】
> DP 表示は計算機シミュレーター実装まで対象外。現時点では残タスクではなく将来 WIP として保留。
- [ ] SP バッジに加え DP バッジを追加（`currentDp / baseMaxDp` 表示）
- [ ] `member.dpState` から `formatDpStateSummary()` 相当を実装

完了条件:
- [x] 各キャラアイコン下でバフ/鬼神化状態が確認できる
- [ ] DP 表示（計算機シミュ WIP のため保留）

> 🔶 T14 部分完了: T14-A（バフアイコン表示・鬼神化/行動不能を通常アイコン群へ統合）実装済み。T14-B（DP表示）は計算機シミュレーター実装まで保留。
