...
2026-05-23 には、`慧眼の女教皇` クイーンの `アトミックフレア` を実データ回帰として追加し、
スキル本体の `IgnoreEShieldElement` が active な Eシールド敵に対して Eシールド減算と
弱点命中扱いの両方を成立させ、`1MORE` の追加ターンへ接続することを固定した。
同日、`絶界に屹立せし蝕樹` のような `extra_gauge.hp` 複数本 + Eシールド敵で、
HPゲージ破壊時に depleted (`current=0`) の Eシールドも次ゲージ開始時に `max` まで復帰する
回帰を追加した。`enemies.json` の raw `extra_gauge.esp` は単一値だが、
`dimension_battle.json` の `eg[0].pattern.hp` には HP break 段階ごとの enemy skill list があり、
Dimension 09 の円環系は追加 skill ID による段階差分を持つ。一方、Dimension 14 の蝕樹は
共通の shield break skill と evolve skill のみで、Eシールド値を増やす明示情報は見えない。
...
- スキル本体の part として持つ `IgnoreEShieldElement` も action-time で評価し、
  active な Eシールド敵に命中した場合は属性一致がなくても Eシールドを減らす
- `IgnoreEShieldElement` を持つアクティブ攻撃が active な Eシールド敵に命中した場合、
  `actionHitsEnemyWeakness()` / `IsHitWeak()` でも弱点命中扱いにする。これにより
  `アトミックフレア` から `1MORE` (`AdditionalHitOnWeak -> AdditionalTurn`) が発火する
...
- [x] `help/HEAVEN_BURNS_RED/バトル/アトミックフレアと1MORE.md` を追加し、`help/scripts/populate_templates.py` の `SKIP_LIST` に登録する
...
- [../../help/HEAVEN_BURNS_RED/バトル/アトミックフレアと1MORE.md](../../help/HEAVEN_BURNS_RED/バトル/アトミックフレアと1MORE.md)
