...
`.git/hooks/pre-commit` は機密情報チェック後、`scripts/code-review-graph-local update` と `scripts/code-review-graph-local detect-changes --brief` を実行する。Code Review Graph の失敗で commit 自体は止めない。
