...
- [ ] **T3-1. `rules[]` の構造調査(対象は `id >= 145000088` のイベントのみ)**
  - [ ] score_attack.json 読み込み・パース処理に `id >= 145000088`(`in_date >= 2025-12-26`)の
        イベントのみを対象とするフィルタを設ける(Phase 1・2 の難易度パラメータ解決部分にも
        同様のフィルタを適用するか要検討。現状 Phase 1・2 は「該当ラベルが登場する最新イベント」を
        自動選択する実装のため、対象外イベントしか無い古い敵を選んだ場合の扱いを確認する)
