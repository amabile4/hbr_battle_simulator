...
### Phase 2: 実装（Codex WBS 差分反映）
- **T2-1**: **記録側** — 新規 snapshot 生成
  （`#buildTurnStartEnemyOverrideEntries` / `buildEnemyStateOverrideSnapshot`）で
  `isRecomputableEnemyStatusPower` な enemy status を override から **除外**。
- **T2-2**: **適用側（必須・記録側だけでは既存 session が直らない / Codex 指摘1・5）** —
  `applyEnemyStateOverrideSnapshot` / `buildOverrideEnemyStatuses` で、
  **apply-time に旧 snapshot の recomputable status を無視**し carry-forward を優先。
  → load-time 破壊的書換ではなく **apply-time 互換**（新規 save は filtered snapshot で自然収束）。
- **T2-3**: predicate 単体テスト + 旧 session 互換テスト（stale snapshot fixture を load/recalc）。

### Phase 3: テスト・検証（predicate 代表方式 / Codex 指摘6）
全 statusType × 6 局面の直積は重いので **predicate ごとの代表**を置く:
- **T3-1**: Phase0 赤テスト（todo）を外して緑化。
- **T3-2 recomputable 代表**: DefenseDown / Fragile / ResistDown(or Overwrite) /
  HighBoost・効果量アップ絡み 1 件 → 上流編集で OD3・追加ターン・通常跨ぎとも追従。
- **T3-3 preserved lifecycle**: Break / DownTurn / Dead / SuperBreak(Down) が凍結維持されること。
- **T3-4 power-is-duration**: Provoke/Attention/Misfortune/Cover から 1-2 件、消えない/再導出されない。
- **T3-5 unknown numeric power**: preserve されること（安全側）。
- **T3-6 old session 互換**: stale snapshot fixture を load→recalc で追従。
- **T3-7**: Playwright（ui-next）で「過去ターン編集→反映→後続ターンのデバフ反映」E2E。
- **T3-8**: session save→load→recompute 往復 / 自バフは安全のまま（非対称性の回帰固定）。
