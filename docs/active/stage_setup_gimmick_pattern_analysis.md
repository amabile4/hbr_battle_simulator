...
- [x] #3 ODゲージ上昇量+20%（`ODピアス` と同じ補正枠へ加算する形で実装。Stage Setup 上段で任意値へ手編集可能）
...
- `Stage Setup` の直入力項目として `ODゲージ上昇量（%）` を追加した
- 恒星戦プリセット `ODゲージ上昇量+20%` は、他の SP/OD 系プリセットと同じく上段入力欄へ `20` を転記する
- 保存・実行時は上段入力欄の値を正本とし、既存 schema の `stageSetup.enchantEffects[*].effectType === 'odGaugeGainBonusPercent'` として保持する
- runtime の OD 計算仕様は既存どおりで、任意 `amount` を `ODピアス` と同じ補正枠へ加算する
