...
- エンシェントチェーン:
  ...
  - 破壊率上昇量+10% はヒット数依存表記がないためフラット加算とし、
    `calculateDestruction.attacker.flatDestructionRateBonus=0.1` として
    `baseDestruction = bg30 * (1 + sRatio + buffMultiplier + flatDestructionRateBonus)` に加算する
...
| ダメージ/破壊率 | `turn-controller.js` / `damage-calculation-context.js` / `char-detail-popup.js` / `destruction-calculator.js` | チェーン攻撃+10%をアクセサリ攻撃枠へ、チェーン破壊率+10%をフラット破壊率加算へ接続 |
...
- `tests/pierce-damage-integration.test.js` — HP/DP効き分け・通常攻撃除外・破壊率傾斜・チェーン破壊率フラット加算
