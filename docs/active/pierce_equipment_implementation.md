...
破壊率上昇量+10%の実計算適用は hbr_calc 側の計算機コアPR待ちで、本リポジトリでは供給フィールドまでを先行実装する。
...
- エンシェントチェーン:
  ...
  - 破壊率上昇量+10% はヒット数依存表記がないためフラット加算案とし、
    `chainDestructionRateBonus=0.1` を damageContext まで供給する。実計算の受け口は hbr_calc 側PR後に同期する
...
| ダメージ/破壊率 | `turn-controller.js` / `damage-calculation-context.js` | チェーン攻撃+10%をアクセサリ攻撃枠へ接続。チェーン破壊率+10%は `chainDestructionRateBonus` として供給のみ先行し、計算適用は hbr_calc 側PR待ち |
...
- `tests/damage-breakdown.test.js` / `tests/damage-calculation-context.test.js` — チェーン攻撃+10%のラベル・context保持
...
- エンシェントチェーンの破壊率上昇量+10%の実計算適用。設計案は `calculateDestruction.attacker.flatDestructionRateBonus` を追加し、非通常/非追撃スキルの `baseDestruction = bg30 * (1 + sRatio + buffMultiplier + flatDestructionRateBonus)` に加算する。hbr_calc 側PR・同期後に `chainDestructionRateBonus` の消費を有効化する
