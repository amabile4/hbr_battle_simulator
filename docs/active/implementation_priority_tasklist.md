...
>
> **判断メモ**: `PRI-015` を完了し、enemy-side `SpecialStatusCountByType(3/22/172)` と enemy status report false positive を解消した。現行 survey の真の残件は `state_condition` `0`, `overwrite_cond` `0`, 敵状態異常 `10 keys / 40 occurrences`
...
4. [`enemy_residual_status_implementation_tasklist.md`](enemy_residual_status_implementation_tasklist.md)
5. [`enemy_status_implementation_tasklist.md`](enemy_status_implementation_tasklist.md)
6. [`passive_implementation_tasklist.md`](passive_implementation_tasklist.md)
7. [`../20260306_tasklist/implementation_status.md`](../20260306_tasklist/implementation_status.md)
8. [`../archive/20260314_priority_history_pri010_012.md`](../archive/20260314_priority_history_pri010_012.md)
...
| P0 | `PRI-016` | `todo` | 未接続 enemy status 残件（確率系 / 補助 debuff / enemy buff / passive enemy debuff） | [`enemy_residual_status_implementation_tasklist.md`](enemy_residual_status_implementation_tasklist.md), [`enemy_status_implementation_tasklist.md`](enemy_status_implementation_tasklist.md), [`special_status_implementation_tasklist.md`](special_status_implementation_tasklist.md) | `PRI-015` 後の残件は `StunRandom` / `ConfusionRandom` / `ImprisonRandom` / `Misfortune` / `HealDown` / `Hacking` / `Cover` / enemy-target `AttackUp` / `DefenseUp` / passive `DefenseDown` に限定された。ここから先は deterministic rule と保持スキーマの整理が主作業になる | 残り 10 key の simulator rule / state 保存方針が決まり、必要なものは preview / commit / report / tests まで接続される |
...
- [ ] `StunRandom` / `ConfusionRandom` / `ImprisonRandom` の deterministic simulator rule を決める
- [ ] `Misfortune` / `HealDown` / `Hacking` / `Cover` の保持スキーマと record 表現を決める
- [ ] enemy-target `AttackUp` / `DefenseUp` と passive `DefenseDown` を action / passive timing へ接続する
- [ ] [`special_status_implementation_tasklist.md`](special_status_implementation_tasklist.md) の `T14`（拘束状態手動フック）をここへ吸収するか再判断する
