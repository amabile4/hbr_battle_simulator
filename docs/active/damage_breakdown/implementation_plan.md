...
> **ステータス**: ✅ 完了 | **最終更新**: 2026-06-03

## Goal

キャラクター詳細ポップアップへ「威力詳細」タブを追加し、現在の行動で採用されたバフ・デバフ・相性を公式カテゴリに沿ったグループの2列テーブルで表示する。
...
- `buff`: 攻撃バフ、属性攻撃バフ、チャージ、フィールド、心眼、アクセサリ。心眼は弱点スキル攻撃時のみ表示し、通常攻撃では除外する。
- `crit-mindeye`: クリティカル基礎倍率、クリティカル威力
- `funnel`: 連撃数アップ
- `token-passive`: トークン攻撃倍率、オギャり、歌姫、固有マークなど
- `debuff`: 防御ダウン、属性防御ダウン、属性耐性ダウン、属性耐性打ち消し、脆弱
