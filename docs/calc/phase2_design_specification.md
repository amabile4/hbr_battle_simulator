...
### ① 攻撃バフ・属性バフ枠の集約
サポートされているバフの種類（`statusType`）は、`AttackUp`（攻撃力アップ）、`Charge`（チャージバフ）、`ElementAttackUp`（属性攻撃バフ）です。
...
### ③ 脆弱デバフ枠の集約
脆弱デバフ（`statusType == 'Fragile'`）は、防御デバフとは別乗算枠（`vulnerabilityMultiplier`）として集約されます。
...
### ④ クリティカル・心眼・連撃枠
- **クリティカル・心眼枠** (`critMindeyeMultiplier`): クリティカル時の基礎ダメージ倍率 `1.5` (150%) に対し、追加のクリティカル威力バフ（`CritDamageUp` / `CritBuff`）および心眼バフ（`MindEye`）を加算した倍率補正 $\frac{1.5 + CritBuff + MindEye}{1.5}$ です。
  * ※ 心眼は弱点攻撃時のみ加算されます（非弱点時は `0` として処理）。
