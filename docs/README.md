...
| [active/t34_enemy_status_management_plan_wbs.md](active/t34_enemy_status_management_plan_wbs.md) | 🟢 進行中 | T34（敵状態変化管理・表示）専用の実装プラン/WBS。未コミット/コミット表示のプレビュー回帰に加え、TurnEdit の `Enemy` ラベルを「敵状態確認」へ変更し、toolbar へ「敵状態確認」「キャラクターアイコン」の簡易ヘルプ（クリック/右クリック/長押し）を追加し、turn row 側の「敵状態確認」を単体ボタン化してクリック誘導性を強化。残タスクは enemy 関連メニュー統合と残E2E | 2026-04-05 |
