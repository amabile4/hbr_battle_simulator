...
| [active/code_review_graph_workspace_setup.md](active/code_review_graph_workspace_setup.md) | ✅ 完了 | Code Review Graph を workspace の `.venv` だけで起動する運用。`uvx` / global PATH 依存を避けるための local wrapper・pre-commit hook・npm scripts を `scripts/code-review-graph-local` 経由へ統一。MCP の絶対パス設定は各ユーザーのローカル設定で管理 | 2026-04-28 |
