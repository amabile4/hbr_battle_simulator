...
| [active/t16b_summon_enemy_slot_wbs.md](active/t16b_summon_enemy_slot_wbs.md) | 🟢 進行中 | T16-B Summon / 敵スロット管理の実装プラン。enemy slot 正本化の基盤に続き、手動 Summon、replay/slot snapshot 反映、popup の耐性/吸収表示、sample enemy 3体固定、enemy detail popup の `E1/E2/E3` 3 tab、wide 3列 / narrow 1列 layout、`名称` fold header、`敵情報確認 / 敵情報 / 敵` label、popup 内 break/kill attribution sub-editor と `ActionOutcomeOverrides` 復帰に加え、stale `enemyCount` で E2 が `未使用` に戻る回帰と draft popup の phantom E3 回帰を修正済み。残りは敵行動データからの自動 summon 化 | 2026-04-08 |
