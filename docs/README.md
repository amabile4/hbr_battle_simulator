...
| [active/e_shield_preparation_plan.md](active/e_shield_preparation_plan.md) | 🟢 進行中 | Eシールド engine-first 実装の正本。inactive raw 定義の ingest 正規化、`turn-controller` の減算 / same-action auto BREAK / `IgnoreEShieldElement`、`eShieldStateByEnemy` 保持、関連 unit test 固定まで完了し、後続は通常 enemy をカテゴリ定義から追加できる Enemy Setup selector 汎用化、手動編集、`HealEShield` / `ReviveEShield` / browser E2E を管理 | 2026-04-19 |
