...
| [active/hybrid_break_next_session_prompt.md](active/hybrid_break_next_session_prompt.md) | 🟢 進行中 | **次セッション起動用プロンプト**。オーケストレーション体制（Fable 5=受入レビュー+最難度タスクA、sonnet/haiku/opusサブエージェント=B〜E）、前提知識・制約・タスクA(HP累積討伐予測)/B(事前予測バッジ)/C(比較ビュー)/D(一時プレビュー)/E(docs収束)の指示を集約 | 2026-06-12 |
| [active/hybrid_auto_manual_break_implementation_plan.md](active/hybrid_auto_manual_break_implementation_plan.md) | 🟢 進行中 | **ハイブリッド運用の採用版実装計画（第2稿ベース）**。操作イベント正本・自動計算ガイド化・一時プレビュー/比較ビュー非永続・連戦召喚前倒し防止を確定方針とし、優先順位付きTODO（T1〜T10）、機能接続チェック表、非接続ポイント、GOAL定義（G1〜G4）、テスト計画を集約。suppression永続化（第1稿案）は不採用。**T1〜T4(DP分)完了**: 連戦召喚整合/保存純度テスト、`resolvePerHitDpDamageByEnemy` によるDP累積消費・DP0自動ブレイク（手動優先）、app起動時データ注入まで実装済み。残: 討伐予測(HP累積)・T5ガイドバッジUI・T6/T7一時状態・T9 E2E | 2026-06-11 |
