...
| [active/three_checkpoint_test_oracle_remediation_wbs.md](active/three_checkpoint_test_oracle_remediation_wbs.md) | ✅ 完了 | 3基準点監査で検出した破壊率oracle追随、stats境界、外部session、lockfile、生成fixture、称号master依存の復元・再現性修正WBS | 2026-07-02 |
...
| [active/effect_up_multiplier_connection_wbs.md](active/effect_up_multiplier_connection_wbs.md) | 🟢 進行中 | シャルロッタ「侵食」未接続を発端に、ag(Gemini)へ効果アップ系skill_type（`GiveDefenseDebuffUp`/`GiveAttackBuffUp`/`GiveHealUp`/`DamageRateUp`/`GiveDebuffUp`）の calc 未接続を包括調査依頼。turn-controller.js の対応resolver（`resolveEnemyDebuffSkillEffectMultiplier`等）を関数実在確認のうえPhase0-4のWBSを記録。`resonance_ability_connection_tasklist.md` の共鳴アビリティ限定残タスクを統合予定 | 2026-07-02 |
| [active/skill_type_icon_rename_pr24_acceptance.md](active/skill_type_icon_rename_pr24_acceptance.md) | 🟢 進行中 | PR #24（`assets/skill_type/` 命名統一・`SKILL_TYPE_IMAGE_MAP` 新設）の受け入れ記録（マージコミット`a4ff6bd`）。実データ上の全statusType（214件）網羅監査で判明したマッピング誤り・漏れ9種を単純マッピング修正、`AttackUp`/`AttackDown`/`DefenseUp`/`DefenseDown`系8種と`GiveDefenseDebuffUp`等4種の計12種は`COMPOSITE_ICON_MAP`＋`resolveSkillTypeIconCompositeHtml()`によるベース画像+矢印(ArrowBuff/ArrowDebuff)の動的レイヤー合成表示で解決（状態変化詳細タブのみ、ブラウザ実描画確認済み）。残り4種（`ResistUp`/`OverDriveRateUp`/`Attention`/`Zone`、現行データ休眠中）は`tests/skill-type-icon-asset-gaps.test.js`で未接続のまま記録 | 2026-07-03 |
| [active/assets_master_data_replacement_checklist.md](active/assets_master_data_replacement_checklist.md) | 🟢 進行中 | `assets/ui`（20件）・`assets/skill_type`（133件、Motivation各段階代表5件は全て必要と訂正）の入れ替え対象リスト確定。skill_type側は不足ゼロ、現状198件中65件が過剰候補（属性別完パケ35件・動的合成移行済み基本バフ6件・専用画像なしステータス名22件・SuperBreak属性版2件）。22件は個別診断済みで動的合成1件・流用マッピング6件(`HealEp`→`HealSp`実装済み)・自作保護2件・流用先未確定12件に分類。ui側はNone.webp(無属性アイコン)が新規格納対象、Break/dead/defeat/Reinforce/Summon/TokenSetの6件は手作業画像と確認済みで対応表待ち | 2026-07-03 |
| [active/assets_required_files_checklist.md](active/assets_required_files_checklist.md) | 🟢 進行中 | `assets/ui`・`assets/skill_type`に格納すべき正規ファイルの実務用一覧（対応表確定分と自作維持分を分離）。格納リストは`assets_master_data_replacement_checklist.md`の監査結果を実務向けに再構成 | 2026-07-03 |
| [active/assets_custom_files_migration_plan.md](active/assets_custom_files_migration_plan.md) | ✅ 完了 | 対応表に正規ファイルがない自作画像11件（ui:Break/dead/defeat/Reinforce/Summon/TokenSet、skill_type:IceSuperBreak/LightSuperBreak/SuperBreakDown/TokenSet/ZoneUpEternal）を`assets/ui_custom/`・`assets/skill_type_custom/`へ隔離。`src/ui/style-asset-url.js`の解決関数にファイル名ベースの自動分岐を実装し呼び出し元は無変更。npm test 1633件・E2E（既知のenemy preset不整合1件除き）PASS確認済み | 2026-07-03 |
...
| [active/e_shield_preparation_plan.md](active/e_shield_preparation_plan.md) | ✅ 完了 | Eシールド実装の完了記録。inactive raw 定義の ingest 正規化、`turn-controller` の減算 / same-action auto BREAK / `IgnoreEShieldElement` / `HealEShield` / `ReviveEShield`、`eShieldStateByEnemy` 保持、turn row 左パネル / enemy detail popup の shared badge 表示、Enemy Setup の `カテゴリ -> 敵` selector 汎用化、通常攻撃の `OD=7.5% 固定 / Eシールド=raw hit 数` 分離、`Party Setup` 属性ブレスレット selector の runtime / session 配線、闇撃のブレス通常攻撃による Dark Eシールド減少と弱点追加ターン回帰、魔界騎兵起動 operation の Dark Eシールド 6 hit 減算、クイーン `アトミックフレア` の `PenetrationCriticalAttack` 常時弱点扱いと `1MORE`、Fire / Ice / Thunder Eシールド属性無視、LB2/LB3 `Funnel` の `Only` 競合回帰、HPゲージ破壊時のEシールド max 復帰と committed 行表示の次ターン境界、`replayScript.setup` の `NormalAttackElementsByPartyIndex` setup entry 同期、`Dimension_09_X_KaleidoOuroboros` の `テンプレート` 常時表示、Enemy Setup manual Eシールド編集、session save/load / summon 回帰まで反映 | 2026-06-28 |
| [active/special_operation_damage.md](active/special_operation_damage.md) | ✅ 完了 | 総攻撃・魔界騎兵の専用operationダメージ計算。JSON/master由来の部隊STR/DEX平均、貫通確定クリティカル、敵別DP/HP・破壊率・OD反映、計算イベント保持を実装 | 2026-06-28 |
...
| ドキュメント | 概要 |
|-------------|------|
| [20260701_invalid_main_history/README.md](20260701_invalid_main_history/README.md) | 置換前の旧 `main` 固有3コミットを無効参照として固定し、現行 `main`、PR #22、計算機精緻化・ASTリファクタの基準タグを正しい参照先として記録 |
