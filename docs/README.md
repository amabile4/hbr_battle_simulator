...
| [active/talisman_completion_wbs.md](active/talisman_completion_wbs.md) | ✅ 完了 | 霊符状態完成 WBS。`恐怖の叫び` の `AdditionalHitOnExtraSkill + Talisman`、record / `damageContext` 露出、enemy popup の icon / summary / action-flow 表示、関連テスト昇格まで完了 | 2026-04-10 |
| [active/disaster_status_wbs.md](active/disaster_status_wbs.md) | ✅ 完了 | `Disaster / 禍` 実装完了記録。`enemyState.disasterState`、`damageContext` 集約、enemy popup / field chip / char detail 表示、audit baseline 更新、browser E2E まで反映。current live style では `もつれトラップ` + `巻き添え` で初回 `Lv4 / 全能力-28` | 2026-04-10 |
