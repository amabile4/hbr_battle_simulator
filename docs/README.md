...
|-------------|-----------|------|----------|
| [active/stage_setup_gimmick_pattern_analysis.md](active/stage_setup_gimmick_pattern_analysis.md) | 🟢 進行中 | Stage Setup 初期ギミック23項目の分類（A:初期状態注入, B:注入基盤追加, C:新規ロジック）と実装優先順位（Priority 1-3）。OD/SP 系 `enchantEffects` 5件と manual `毎ターンOD（%）` に加え、`毎ターンSP` / turn-start 条件付き SP の T1 反映と `Stage Setup` passive log 出力を shared helper 経由で整理。WBS は 14/23 完了(61%) のまま管理 | 2026-04-22 |
...
| [specs/od_gauge_calculation_spec.md](specs/od_gauge_calculation_spec.md) | ODゲージ計算仕様（通常攻撃 `2.5%` 固定、od_rate の1hit単位切り捨て・共鳴OD重複防止・Stage Setup `ODゲージ上昇量+20%` の `ODピアス` 同枠加算を反映） | 2026-04-20 |
