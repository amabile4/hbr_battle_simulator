...
| [active/ui_next_unimplemented_tasklist.md](active/ui_next_unimplemented_tasklist.md) | 🟢 進行中 | UI Next 単一 backlog。`setup_panel_layout_and_perf` / `implementation_priority` / `skill_limit` / `stage_setup_gimmick` の未実装も統合管理。T33 第1波（実データ監査 / 再現ケース化）は完了し、現優先を `T33-FU1: AdditionalHitOnExtraSkill + Talisman`、次点を `PRI-018 / Skill Usage Limits` と `T16-B auto summon` に更新 | 2026-04-10 |
| [active/t33_skill_passive_audit_wbs.md](active/t33_skill_passive_audit_wbs.md) | 🟢 進行中 | T33 実データ基準監査の WBS。`HbrDataStore` ベースの audit harness・baseline test・doc sync を第1波として完了し、残課題を `T33-FU1`（`恐怖の叫び` の `AdditionalHitOnExtraSkill + Talisman`）へ限定 | 2026-04-10 |
| [active/t16b_summon_enemy_slot_wbs.md](active/t16b_summon_enemy_slot_wbs.md) | 🟢 進行中 | T16-B Summon / 敵スロット管理の実装プラン。enemy slot 正本化の基盤に続き、手動 Summon、replay/slot snapshot 反映、popup の耐性/吸収表示、sample enemy 3体固定、enemy detail popup の `E1/E2/E3` 3 tab、wide 3列 / narrow 1列 layout、`名称` fold header、`敵情報確認 / 敵情報 / 敵` label、popup 内 break/kill attribution sub-editor と `ActionOutcomeOverrides` 復帰に加え、stale `enemyCount` で E2 が `未使用` に戻る回帰、draft popup の phantom E3 回帰、summon 後 selector/break/follow-up/recommit 回帰を修正・固定済み。残りは敵行動データからの自動 summon 化と `BattleStateManager` 責務整理 | 2026-04-10 |
...
| [active/t34_followup_tasklist.md](active/t34_followup_tasklist.md) | 🟢 進行中 | T34 フォローアップ。表示 unit / lifecycle integration は完了扱いへ整理し、残りを WBS-3e（enemy 関連メニュー統合）、browser E2E 残件、T34-FU1（per-source instance 管理）に絞り込み | 2026-04-10 |
| [active/lightweight_record_replay_design.md](active/lightweight_record_replay_design.md) | 🟢 進行中 | 軽量 replay/edit 設計案。ReplayScript 正本化自体は完了済みと整理し、残タスクを auto summon / override 正規化まわりの追加固定 / `BattleStateManager` 責務整理へ更新 | 2026-04-10 |
...
| [active/stateful_passive_wbs.md](active/stateful_passive_wbs.md) | 🟢 進行中 | 状態付与型パッシブ（AdditionalHit* 38件）の実装WBS。T33 第1波監査を反映し、`浄化の喝采`・`AdditionalHitOnBreaking + AttackUp`・`DebuffGuard`・`BuffCharge` を既実装へ訂正。残課題は `恐怖の叫び` の `AdditionalHitOnExtraSkill + Talisman` のみ | 2026-04-10 |
| [active/passive_test_coverage_audit.md](active/passive_test_coverage_audit.md) | 🟢 進行中 | パッシブ発火トリガー × exitCond テストカバレッジ監査。T33 audit harness / baseline test を反映し、runtime gap 1件・observability gap 2件・stale doc false positive 5件・out-of-scope 3件へ再分類 | 2026-04-10 |
