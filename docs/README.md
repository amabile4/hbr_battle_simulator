...
| [active/partysetup_stats_editing_plan.md](active/partysetup_stats_editing_plan.md) | 🟢 設計確定 | PartySetupステータス編集機能。現状は計算機がrole標準値プレースホルダ固定で実stats入力手段が無く、3点一致検証のクリティカルパス。P-0確定: スロット単位(statsByPartyIndex・lbと同じ持ち方)・別パネルoverlay・role標準プリフィル・サポート枠対応・完全自由入力(最終6数値の手入力のみ、装備/宝珠/称号等の連動なし)。CharacterStyle→計算機供給＋session永続化。Excel非依存で実装可。P-1〜P-6 | 2026-06-04 |
