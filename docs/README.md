...
| [active/passive_implementation_tasklist.md](active/passive_implementation_tasklist.md) | 🟢 進行中 | パッシブ条件・タイミング実装計画。`AdditionalHitOnExtraSkill` の `DebuffGuard` / `BuffCharge` / `Talisman` を含む trigger 経路、passive timing の `BuffCharge` 実状態付与、`NegativeMind` の `NegativeState(146)` 初戦開始付与と `RemoveDebuff` 解除、`Makeup` の `Makeup(164)` 初戦開始付与、`Mocktail` の `Mocktail(313)` 状態付与とDP回復量補正、料理バフ4種（`Curry` / `Shchi` / `Steak` / `Gelato`）の Eternal 状態・スキル攻撃補正・`HealDpByDamage` record 接続、`Babied(258)` の自身以外3T付与・スキル攻撃/OD上昇補正を実装済みとし、残課題を `ConquestBikeLevel` UI override・印/UI 観測強化・OD終了側追跡へ整理 | 2026-05-16 |
...
| [active/passive_test_coverage_audit.md](active/passive_test_coverage_audit.md) | 🟢 進行中 | パッシブ発火トリガー × exitCond テストカバレッジ監査。2026-05-17 再照合で runtime gap 0件・observability gap 2件・stale doc false positive 0件・out-of-scope 3件を維持し、style-embedded passive 120件を `HbrDataStore` 監査必須として明記 | 2026-05-17 |
...
| [specs/od_gauge_calculation_spec.md](specs/od_gauge_calculation_spec.md) | ODゲージ計算仕様（通常攻撃 `2.5%` 固定、od_rate の1hit単位切り捨て・共鳴OD重複防止・Stage Setup `ODゲージ上昇量（%）` 任意値の `ODピアス` 同枠加算を反映） | 2026-04-27 |
