...
| [active/partysetup_stats_editing_plan.md](active/partysetup_stats_editing_plan.md) | ✅ 完了 | PartySetupステータス編集機能。スロット単位の `statsByPartyIndex`、メイン/サポート別パネル、role標準プリフィル、自由入力、CharacterStyle・session・lightweight replay 永続化、威力詳細への実stats供給を実装。サポート枠は公式ヘルプ調査に基づき各ステータス10%を加算。unit / Playwright 回帰まで固定済み | 2026-06-04 |
