...
| [active/damage_breakdown/unimplemented_elements_wbs.md](active/damage_breakdown/unimplemented_elements_wbs.md) | 🟢 進行中 | 威力詳細 v1 に掲載されていないがダメージ計算に関与する要素の洗い出し。Priority 1（食事バフ攻撃力・ハイブースト・トークン連動ダメージアップ）と `isDestructionRateGainSkill` / 固有マーク必殺技倍率の掲載まで反映済み。全能力ダウンは計算入力へ `paramBorder` 補正として接続済み、倍率内訳表示は後続。残りは source 付き HighBoost contribution 化、アクセサリ基盤、DP条件詳細 | 2026-06-07 |
