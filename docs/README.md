...
| [active/damage_breakdown/implementation_plan.md](active/damage_breakdown/implementation_plan.md) | ✅ 完了 | キャラクター詳細ポップアップの5つ目タブとして、対象敵ごとのクリティカル時スキル威力予測倍率と7グループ内訳、欄外クリティカル発生率を表示する実装完了記録 / WBS | 2026-05-31 |
| [active/damage_breakdown/unimplemented_elements_wbs.md](active/damage_breakdown/unimplemented_elements_wbs.md) | 🟢 進行中 | 威力詳細 v1 に掲載されていないがダメージ計算に関与する要素の洗い出し。Priority 1（食事バフ攻撃力・ハイブースト・トークン連動ダメージアップ・全能力ダウン）と `isDestructionRateGainSkill` / 固有マーク必殺技倍率の掲載まで反映済み。残りは source 付き HighBoost contribution 化、任意の Talisman/Disaster 内訳、アクセサリ基盤、DP条件詳細 | 2026-05-31 |
