...
| [active/assets_master_data_replacement_checklist.md](active/assets_master_data_replacement_checklist.md) | 🟢 進行中 | `assets/ui`（20件）・`assets/skill_type`（129件）の入れ替え対象リスト確定。skill_type側は不足ゼロ、現状198件中69件が過剰候補（属性別完パケ35件・動的合成移行済み基本バフ6件・専用画像なしステータス名22件・Motivation代表以外4件・SuperBreak属性版2件）に分類。ui側はNone.webp(無属性アイコン)が新規格納対象、Break/dead/defeat/Reinforce/Summon/TokenSetの6件は手作業画像と確認済みで対応表待ち | 2026-07-03 |
