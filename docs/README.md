...
| [active/implementation_priority_tasklist.md](active/implementation_priority_tasklist.md) | 🟢 進行中 | `ui-next` の manual break attribution / session save-load 波に続き、runtime/data-store 側の `SkillSwitch` selectable variant 展開と `styles.json` 埋め込み skill fallback を反映し、残 correctness backlog として PRI-018 を次優先に再整理。 | 2026-03-20 |
| [active/ui_next_design.md](active/ui_next_design.md) | 🟢 進行中 | `Initial Setup` と `Simulator Settings` の責務分離、manual target UI の info-space 配置、single-target の target/break 一体 editor、special operation chips、Party Setup の preset overwrite confirm と `SP +3` 既定値に加え、runtime 正本の selectable skill list（`SkillSwitch` variant 展開・埋め込み `通常攻撃`/`指揮行動`/`追撃` fallback）まで整理した UI Next 設計メモ | 2026-03-20 |
