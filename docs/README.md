...
| [active/t16b_summon_enemy_slot_wbs.md](active/t16b_summon_enemy_slot_wbs.md) | 🟢 進行中 | T16-B Summon / 敵スロット管理の実装プラン。監査で判明した `enemyCount` 依存、kill 時の `Dead` 未反映、enemy[0] 固定の `od_rate`、alive enemy 条件判定の論点を整理し、設計→kill/state修正→OD per-enemy 化→条件判定統一→UI/replay→テストの順で WBS 化 | 2026-04-06 |
