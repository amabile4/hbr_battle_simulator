...
| [active/hybrid_auto_manual_break_wbs.md](active/hybrid_auto_manual_break_wbs.md) | 🟢 進行中 | 自動計算と手動計算のハイブリッド運用で、再計算・JSON保存/読込時にブレイク/HP破壊/討伐が衝突しないためのWBS。現状の問題点（自動抑止の永続化欠如、優先順位仕様不足）と、次AI向けのデータモデル拡張・エンジン適用・UI・テスト・移行提案を記載 | 2026-06-10 |
