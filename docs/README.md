...
| [active/destruction_rate_implementation_plan.md](active/destruction_rate_implementation_plan.md) | 🟢 進行中 | 破壊率（HPダメージ係数・最大1299%級）。エンジン単体（calculateDestruction）実装済み、右クリックポップアップ手動入力（暫定）完了。D-3 turnState最小接続、敵 `d_rate` 上昇倍率接続、EnemySetup手動入力・snapshot往復・`destructionMultiplierByEnemy` 反映、威力詳細手入力計算への倍率反映、D-4 damageContext 接合、D-5 HP ダメージ表示、D-6エッジ検証、DP破壊率除外、コードダクネス9hit検証、新規snapshotの敵DP配線、超越capの威力詳細手入力反映を反映。威力詳細での多段中DP→HP按分表示は後続 | 2026-06-13 |
...
| ドキュメント | ステータス | 概要 | 最終更新 |
|-------------|-----------|------|----------|
| [active/pierce_equipment_implementation.md](active/pierce_equipment_implementation.md) | ✅ 完了 | ピアス装備4種（ドライブ/アタック/ブレイク/ブラスト × 10/12/15%）のヒット数補正実装。減衰型（アタック=対HP・ブレイク=対DPダメージ乗数）と上昇型（ブラスト=破壊率上昇量・ドライブ=OD上昇量）を `src/domain/pierce-correction.js` に共通化し、PartySetup `pierce` select → snapshot `pierceByPartyIndex` → CharacterStyle → damageContext → calculateDamage / calculateDestruction まで接続。旧 `drivePierceByPartyIndex` snapshot/preset 互換維持。残課題はエンシェントチェーン・共鳴破壊率・敵 `d_rate` 入力 | 2026-06-13 |
