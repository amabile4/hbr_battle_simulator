...
| [active/skill_type_icon_rename_pr24_acceptance.md](active/skill_type_icon_rename_pr24_acceptance.md) | 🟢 進行中 | PR #24（`assets/skill_type/` 命名統一・`SKILL_TYPE_IMAGE_MAP` 新設）の受け入れ記録（マージコミット`a4ff6bd`）。マージに伴い `tests/enemy-status-display.test.js` の旧ファイル名アサーションを `BuffDefense.webp` へ追従修正。分類A（マッピング先未提供6種: `HealSp`/`SpecifySp`/`HealEp`/`OverDrivePointUp`/`OverDrivePointDown`/`ResistDown`）・分類B（旧ファイル削除済みで代替未マッピング6種: `BreakDownTurnUp`/`GiveDefenseDebuffUp`/`HealDpByDamage`/`Provoke`/`RegenerationDp`/`ReviveDpRate`）の計12 statusTypeを `tests/skill-type-icon-asset-gaps.test.js` で可視化。統合的マッピングは意図的に行わず未接続のまま記録する方針 | 2026-07-02 |
