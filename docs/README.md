...
| [active/e_shield_preparation_plan.md](active/e_shield_preparation_plan.md) | 🟢 進行中 | Eシールド engine-first 実装の正本。inactive raw 定義の ingest 正規化、`turn-controller` の減算 / same-action auto BREAK / `IgnoreEShieldElement`、`eShieldStateByEnemy` 保持に加え、turn row 左パネル / enemy detail popup の shared badge 表示と browser E2E まで完了。後続は Enemy Setup selector 汎用化、手動編集、`HealEShield` / `ReviveEShield` を管理 | 2026-04-19 |
