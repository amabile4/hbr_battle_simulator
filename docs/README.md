...
| [active/hybrid_auto_manual_break_implementation_plan.md](active/hybrid_auto_manual_break_implementation_plan.md) | 🟢 進行中 | **ハイブリッド運用の採用版実装計画（第2稿ベース）**。操作イベント正本・自動計算ガイド化・一時プレビュー/比較ビュー非永続・連戦召喚前倒し防止を確定方針とし、優先順位付きTODO（T1〜T10）、機能接続チェック表、非接続ポイント、GOAL定義（G1〜G4）、テスト計画を集約。suppression永続化（第1稿案）は不採用 | 2026-06-11 |
