...
| ドキュメント | ステータス | 概要 | 最終更新 |
|-------------|-----------|------|----------|
| [active/master_json_gap_analysis.md](active/master_json_gap_analysis.md) | 🟢 進行中 | `golden/master_json` 集合A（キャラ・カード・スキル・能力）と `json/` の突き合わせ調査。`spListByUsedCount`（SP段階増加）と `intervalTurn`（クールダウン）を hbr_analysis PR#2 で対応済み。シミュレーター側の T-A/T-B 実装タスク、INTRINSIC_MARK_EFFECTS 等のハードコード確認タスクを整理 | 2026-06-27 |
