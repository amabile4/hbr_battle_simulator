...
| [active/skill_type_icon_rename_pr24_acceptance.md](active/skill_type_icon_rename_pr24_acceptance.md) | 🟢 進行中 | PR #24（`assets/skill_type/` 命名統一・`SKILL_TYPE_IMAGE_MAP` 新設）の受け入れ記録（マージコミット`a4ff6bd`）。実データ上の全statusType（214件）網羅監査により、マッピング誤り・漏れ13種（`HealSp`等8種に加え、矢印込み専用ファイルがあるのに矢印なしベース素材へ丸められバフ/デバフが同一画像になっていた`AttackUp`/`AttackDown`/`DefenseUp`/`DefenseDown`系5種、ライブ発火実績のある`NegativeState`→`NegativeMind`異名同義語1種）を修正し`tests/skill-type-icon-asset-gaps.test.js`で回帰確認。残り8種（レイヤー合成待ち4種+専用アセット未提供・現行データ休眠中4種`ResistUp`/`OverDriveRateUp`/`Attention`/`Zone`）は未接続のまま同テストで記録 | 2026-07-02 |
