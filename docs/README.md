...
| [active/skill_type_icon_rename_pr24_acceptance.md](active/skill_type_icon_rename_pr24_acceptance.md) | 🟢 進行中 | PR #24（`assets/skill_type/` 命名統一・`SKILL_TYPE_IMAGE_MAP` 新設）の受け入れ記録（マージコミット`a4ff6bd`）。マージ直後に `tests/enemy-status-display.test.js` の旧ファイル名アサーションを `BuffDefense.webp` へ追従修正。実ファイル確認により当初検出した12 statusType未接続のうち8種（`HealSp`/`SpecifySp`/`HealEp`/`OverDrivePointUp`/`OverDrivePointDown`/`ResistDown`/`BreakDownTurnUp`/`Provoke`）は `SKILL_TYPE_IMAGE_MAP` の誤り・漏れと判明し修正、`tests/skill-type-icon-asset-gaps.test.js` で回帰確認。残り4種（`GiveDefenseDebuffUp`/`HealDpByDamage`/`RegenerationDp`/`ReviveDpRate`）はレイヤー合成の実装待ちとして同テストで未接続を記録 | 2026-07-02 |
