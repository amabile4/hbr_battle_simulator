...
interface DamageInput {
  ...
  attacker: {
    ...
    styleId: number;
    /** 攻撃者のレベル。Party Setup の自動テンプレートではLv200固定 */
    level?: number;
    /** 限界突破段階 (0〜4凸)。ステータス自動補完の精度を担保するために強く推奨 (デフォルト: 0) */
    limitBreakCount?: number;
    /** 
     * 力、器用さ、知性、精神、運、体力の解決済みステータス。
     * Party Setup 未入力時は、戦闘初期化前にテンプレート①と限界突破数から自動算出されます。
     */
    stats?: {
      str: number;
      dex: number;
...
* **動作仕様**:
  - Party Setup の未入力値を、戦闘初期化時に `characters.json` / `styles.json` から具体化して `attacker.stats` に渡します。
  - テンプレート①は **Lv200・転生5回・能力ボード最大・未選択の同キャラスタイル完凸・装備なし**を仮定します。宝珠ボードの能力加算は含みますが、装備、称号、ダフネ、期間/stage補正、マスタースキル等の条件付き能力上昇は含みません。
  - ユーザーが指定したメイン／サポートの限界突破数を、選択スタイルの `stat_up_per_level` と同キャラ共有能力の双方へ反映します。サポート枠がある場合は各能力の10%を切り上げて加算します。
...
ユーザーがゲーム内で確認した最終ステータスを手動入力し、ダメージを精密に合わせたい場合のケースです。
* **動作仕様**:
