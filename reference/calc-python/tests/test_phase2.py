...
class TestPhase2Logic(unittest.TestCase):
    ...
    def test_resolve_effect_power_scaling_wis(self):
        ...
        self.assertAlmostEqual(val_high, 78.11128, places=4)
    ...
    def test_destruction_rate_override(self):
        """
        destructionRateOverrideが指定されている場合、現在の破壊率よりも優先してダメージ計算に適用されること
        """
        input_data = {
            "attacker": {
                "characterId": "手塚咲",
            ...
            "defender": {
                "enemyId": 13000001,
                "destructionRate": 1.5,          # 通常の破壊率 150%
                "destructionRateOverride": 3.5,  # 一時上書き破壊率 350%
                "statusEffects": []
            },
            "skill": {
                "name": "通常攻撃"
        ...
        # 破壊率なし（1.0倍）の時のダメージを基準にして、3.5倍になっていることを検証
        input_base = dict(input_data)
        input_base["defender"] = {
            "enemyId": 13000001,
            "destructionRate": 1.0,
            "statusEffects": []
        }
        res_base = self.engine.calculate_damage(input_base)
        
        self.assertAlmostEqual(res["normal"]["expected"], res_base["normal"]["expected"] * 3.5, places=4)

    def test_over_limit_growth(self):
        """
        パラメータ限界値超過時の挙動テスト (Option B)
        """
        # 手塚咲 (通常攻撃: min=237.5, max=475.0, E=100.0)
        # Enemy Border = 300
        # 限界値の閾値ステータス = 300 + 100 = 400
        # 超過ステータスとして 500 を設定 (超過量 = 100)
        input_data = {
            "attacker": {
                "stats": {"str": 500, "dex": 500, "wis": 500, "spr": 500, "luk": 500, "con": 500},
                "statusEffects": []
            },
            "defender": {
                "paramBorder": 300,
        ...
        # 1. clamp_over_limit = True (デフォルト) -> 475.0 にクランプされるはず
        res_clamped = self.engine.calculate_damage(input_data)
        self.assertAlmostEqual(res_clamped["breakdown"]["baseDamageNormal"], 475.0, places=4)

        # 2. clamp_over_limit = False -> 475.0 + 475.0 * 100 * 0.0025 = 475.0 + 118.75 = 593.75 になるはず
        input_unclamped = dict(input_data)
        input_unclamped["options"] = {"clampOverLimit": False}
        res_unclamped = self.engine.calculate_damage(input_unclamped)
        self.assertAlmostEqual(res_unclamped["breakdown"]["baseDamageNormal"], 593.75, places=4)

    def test_debuff_scaling_with_border(self):
        """
        デバフ（脆弱）の効果量が敵境界値との差に基づき正確にスケーリングされること
        """
        # ネイキッドイレイザー (ID: 46001304):
        # min_power = 42%, max_power = 60%, diff_for_max = 132, parameters: wis 1, luk 2
        # スキルLv10:
        # v_min_l = 42 * (1 + 0.0 * 9) = 42.0% (※脆弱効果量自体のレベル成長率は0)
        # v_max_l = 60 * (1 + 0.0 * 9) = 60.0%
        
        # 1. 味方平均ステータス 600, 敵境界値 500 -> 差 = 100
        # 期待値: 42.0 + (60.0 - 42.0) * 100 / 132 = 55.6363...% -> 切り捨てで 55.63%
        eff_mid = {
            "statusType": "Fragile",
        ...
        val_mid = self.engine.resolve_effect_power(eff_mid, enemy_border=500)
        self.assertAlmostEqual(val_mid, 55.63, places=4)

        # 2. 下限クランプ (ステータス差がマイナス -> 下限値に固定)
        eff_low = {
            "statusType": "Fragile",
            "sourceSkillId": 46001304,
            "skillLevel": 10,
            "providerStats": {"wis": 450, "luk": 450}
        }
        val_low = self.engine.resolve_effect_power(eff_low, enemy_border=500)
        self.assertAlmostEqual(val_low, 42.0, places=4)

        # 3. 超過成長 (ステータス差 200 >= 132 -> 60.0% + 60.0% * (200-132) * 0.001 = 60.0 * 1.068 = 64.08%)
        eff_high = {
            "statusType": "Fragile",
            "sourceSkillId": 46001304,
            "skillLevel": 10,
            "providerStats": {"wis": 700, "luk": 700}
        }
        val_high = self.engine.resolve_effect_power(eff_high, enemy_border=500)
        self.assertAlmostEqual(val_high, 64.08, places=4)

        # 4. 宝珠Lv2強化時 (限界値 132 + 40 = 172)
        # 通常分: 42 + ((60 - 42) / 132) * 100 = 55.6363...%
        # 宝珠追加分: (((60 - 42) / 172) * 100 + 42) * 2 * 0.02 = 2.0986%
        # 合計: 55.6363...% + 2.0986% = 57.7349...% -> 切り捨てで 57.73%
        eff_jewel = {
            "statusType": "Fragile",
            "sourceSkillId": 46001304,
            "skillLevel": 10,
        ...
        val_jewel = self.engine.resolve_effect_power(eff_jewel, enemy_border=500)
        self.assertAlmostEqual(val_jewel, 57.73, places=4)

    def test_pierce_equipment_multiplier(self):
        """
        ピアス装備によるダメージ乗数(pierceMultiplier)が、攻撃種別、攻撃対象(HP/DP)に応じて正しく適用されること
        """
        # 基準データ (通常攻撃ではないスキル攻撃)
        base_input = {
            "attacker": {
            ...
            "skill": {
                "name": "星の海、航海の果てに"  # 通常攻撃以外のスキル名
            }
        }

        # 1. 対HPターゲットでのスキル攻撃 (attackPierceUpRate = 0.15 が適用され、乗数 1.15)
        res_hp_skill = self.engine.calculate_damage(base_input)
        self.assertAlmostEqual(res_hp_skill["breakdown"]["pierceMultiplier"], 1.15, places=4)

        # 2. 対DPターゲットでのスキル攻撃 (breakPierceUpRate = 0.12 が適用され、乗数 1.12)
        dp_input = dict(base_input)
        dp_input["defender"] = {
            "enemyId": 13000001,
            "isHpTarget": False,
            "statusEffects": []
        }
        res_dp_skill = self.engine.calculate_damage(dp_input)
        self.assertAlmostEqual(res_dp_skill["breakdown"]["pierceMultiplier"], 1.12, places=4)

        # 3. 通常攻撃 (通常攻撃・追撃には適用されず、乗数 1.00)
        normal_input = dict(base_input)
        normal_input["skill"] = {
            "name": "通常攻撃",
            "isNormalAttack": True
        }
        res_hp_normal = self.engine.calculate_damage(normal_input)
        self.assertAlmostEqual(res_hp_normal["breakdown"]["pierceMultiplier"], 1.00, places=4)

        # 4. 対HPターゲットで breakPierceUpRate のみが指定された場合 (HPターゲットなので適用されず、乗数 1.00)
        hp_only_break_input = dict(base_input)
        hp_only_break_input["attacker"] = {
            "characterId": "手塚咲",
            "styleId": 1010103,
            "stats": {"str": 600, "dex": 600},
            "breakPierceUpRate": 0.12,
        ...
        res_hp_only_break = self.engine.calculate_damage(hp_only_break_input)
        self.assertAlmostEqual(res_hp_only_break["breakdown"]["pierceMultiplier"], 1.00, places=4)

    def test_flat_destruction_rate_bonus(self):
        """
        フラットな破壊率ボーナス(flatDestructionRateBonus)が正しく適用されること
        """
        from destruction_calc_engine import DestructionCalculatorEngine
        dest_engine = DestructionCalculatorEngine()
        ...
        # 1. スキル攻撃時のフラット加算 (base_dest_rate = 0.25 * 10 * (1.0 / 25) = 0.10)
        # flatDestructionRateBonus = 0.10 が加算され、(1.0 + 0.0 + 0.0 + 0.10) = 1.10 倍になる。
        # base_destruction = base_dest_rate * 1.10 = 0.10 * 1.1 = 0.11
        res_skill = dest_engine.calculate_destruction(base_input)
        self.assertAlmostEqual(res_skill["breakdown"]["baseDestruction"], 0.11, places=4)
        self.assertAlmostEqual(res_skill["breakdown"]["flatDestructionRateBonus"], 0.10, places=4)

        # 2. 通常攻撃時 (フラット加算は適用されず、base_dest_rate = 0.25 * 8 * (1.0 / 25) = 0.08 そのまま)
        normal_input = dict(base_input)
        normal_input["skill"] = {
            "name": "通常攻撃",
            "isNormalAttack": True
        }
        res_normal = dest_engine.calculate_destruction(normal_input)
        self.assertAlmostEqual(res_normal["breakdown"]["baseDestruction"], 0.08, places=4)
