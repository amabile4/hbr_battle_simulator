...
- `OverDrivePointUp` を持つスキル（例: 海のギャング、渾身銃撃、サービス・エース、哀のスノードロップ）は、現時点ではOD加算ボーナス未反映。
- 上記4件は `skills.json` 上で `parts.skill_type = OverDrivePointUp` を確認済み。次フェーズで計算式反映・検証を行う。
...
- 単体攻撃のOD増加量:
  - `gain_single = trunc2(h * 2.5 * (1 + bonus/100))`
- 全体攻撃（敵N体）のOD増加量:
  - `per_enemy = trunc2(h * 2.5 * (1 + bonus/100))`
  - `gain_all = trunc2(per_enemy * N)`
