...
test('鬼神化中 STezuka: formatSkillCostLabel がエンジン経由で "SP 0" を返す', () => {
  const state = buildStateWithTezuka();
  if (!state) return; // STezuka スタイルがデータに存在しない場合はスキップ
  ...
  assert.equal(label, 'SP 0', `鬼神化中 STezuka の SP スキルラベルが "SP 0" になる`);
});
...
test('STezuka でも isReinforcedMode=false なら通常コストを返す（state=null）', () => {
  ...
  assert.equal(formatSkillCostLabel(skill, member, null), 'SP 10');
});
...
test('非 STezuka の isReinforcedMode=true: 鬼神化チェックをスキップして通常コストを返す（state=null）', () => {
  ...
  assert.equal(formatSkillCostLabel(skill, member, null), 'SP 4');
});
