...
test('transcendence burst raises destruction gain and cap without stacking with SuperBreak cap bonus', () => {
  ...
  assert.equal(nextState.turnState.enemyState.destructionRateByEnemy['0'], 590.2700000000001);
});
...
test('transcendence burst destruction cap is actor-element gated and does not add to higher stored cap', () => {
  ...
  assert.ok(
    Math.abs(nonMatchingCommitted.nextState.turnState.enemyState.destructionRateByEnemy['0'] - 290.25) < 1e-9
  );
  ...
  assert.ok(
    Math.abs(highStoredCapCommitted.nextState.turnState.enemyState.destructionRateByEnemy['0'] - 690.2700000000001) < 1e-9
  );
});
...
test('same-action SuperBreak 後の破壊率上昇は拡張後 cap を使用する', () => {
  ...
  assert.equal(nextState.turnState.enemyState.destructionRateCapByEnemy['0'], 600);
  assert.equal(nextState.turnState.enemyState.destructionRateByEnemy['0'], 590.25);
});
...
test('手動ブレイク指定はperHitDpDamageのDP残量より優先され、DRが加算されDPが0になる', () => {
  ...
  // 正式式では d_rate=1, 1hit, dr=10 の加算が各1.25%。
  assert.ok(
    Math.abs(dr - 102.5) < 1e-9,
