...
test('comparison view: skullfeather fixture keeps interrupt OD skills and swapped #3 actor', () => {
  ...
  for (const [index, member] of initialState.party.entries()) {
    assert.deepEqual(
      member.stats,
      session.setup.statsByPartyIndex[String(index)].stats,
      `party ${index} の固定main statsがBattleStateまで不変であること`
    );
    assert.deepEqual(
      member.supportStats,
      session.setup.statsByPartyIndex[String(index)].supportStats,
      `party ${index} の固定support statsがBattleStateまで不変であること`
    );
  }
  const manager = new TurnEngineManager();
