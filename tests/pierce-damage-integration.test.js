...
test('calculateDestruction: accessoryDestructionRateBonus が加算グループで破壊率に寄与する', () => {
  ...
  // skill 不明時はフォールバック経路: baseDestRate = 0.04。
  const expectedGain = 0.046;
...
test('calculateDestruction: flatDestructionRateBonus（エンシェントチェーン）はヒット数非依存のフラット加算', () => {
  // skill 不明時は sp=4, dr=1, destMult=1 → baseDestRate = 0.04（破壊済み全ヒット加算）。
  ...
  for (const hitCount of [1, 5, 10]) {
    ...
    assert.equal(withChain.breakdown.flatDestructionBonus, 0.1, `hit=${hitCount}: breakdown`);
  }
});
