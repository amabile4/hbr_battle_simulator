...
test('calculateDestruction: flatDestructionRateBonus はヒット数傾斜なしで破壊率に寄与する', () => {
  ...
  for (const hitCount of [1, 5, 10]) {
    const result = calculateDestruction(buildInput(0.1, hitCount), data);
    assertAlmostEqual(result.destructionRate, 1.044, `hit=${hitCount}: エンシェントチェーン相当`);
    assert.equal(result.breakdown.flatDestructionRateBonus, 0.1);
    assert.equal(result.breakdown.accessoryBonus, 0);
  }
});
